"""
SISTEMA UNIVERSITARIO - SERVIDOR DE API REST EN PYTHON & SQLITE
Servidor liviano sin dependencias externas (usa librería estándar http.server)
"""
...
class UniversityRequestHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def sync_git(self):
        try:
            ...
            subprocess.run(["git", "commit", "-m", "Auto-sync desde Python Backend SQL"], check=False)
            res = subprocess.run(["git", "push", "origin", "main"], capture_output=True, text=True)
            output_msg = res.stdout or res.stderr or "Git sync ok"
            self._set_headers(200)
            self.wfile.write(json.dumps({"status": "success", "output": output_msg}).encode("utf-8"))
...
if __name__ == "__main__":
    ...
    print(f"Servidor Python + API SQLite corriendo en http://localhost:{PORT}")
